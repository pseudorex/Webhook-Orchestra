...
# Create a custom registry to avoid mixing with standard Python library metrics
...
# Redis client for fetching queue metrics dynamically
try:
...
def update_queue_length_metrics():
    """Queries Redis to get current queue lengths and updates the Prometheus gauges."""
    if not redis_client:
        return
